...
function onReset(row: SystemUserApi.SystemUser) {
  const hideLoading = message.loading({
    content: $t('common.resettingPassword', [row.realName]),
    duration: 0,
  ...
  resetUserPassword(row.id)
    .then(() => {
      message.success({
        content: $t('common.passwordResetSuccessful', [row.realName]),
        key: 'action_process_msg',
      });
      onRefresh();
    })
  ...
  <Page auto-content-height>
    ...
    <Grid :table-title="$t('system.user.list')">
      <template #toolbar-tools>
        <AccessControl :codes="['AdminUserCreate']" type="code">
