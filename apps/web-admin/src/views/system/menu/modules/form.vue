...
import { getMenuTypeOptions, SysMenuType } from '../data';
...
const schema: VbenFormSchema[] = [
  ...
  // },
  {
    ...
    dependencies: {
      show: (values) => {
        return ['embedded', SysMenuType.Catalog, SysMenuType.Menu].includes(
          values.type,
        );
  ...
  // },
  {
    ...
    dependencies: {
      rules: (values) => {
        return values.type === SysMenuType.Menu ? 'required' : null;
        // return values.type === 'menu' ? 'required' : null;
      },
      show: (values) => {
        return values.type === SysMenuType.Menu;
  ...
  // },
  {
    ...
    dependencies: {
      ...
      show: (values) => {
        ...
        // return ['button', 'catalog', 'menu'].includes(values.type);
      },
      triggerFields: ['type'],
    },
    fieldName: 'name',
  ...
  //   dependencies: {
  //     show: (values) => {
  //       return [SysMenuType.Menu].includes(values.type);
  ...
  //   dependencies: {
  //     show: (values) => {
  //       return [SysMenuType.Catalog, SysMenuType.Menu].includes(values.type);
...
async function onSubmit() {
  ...
  if (valid) {
    ...
    const data =
      ...
      >();
    data.display_name_cn = data.display_name;
